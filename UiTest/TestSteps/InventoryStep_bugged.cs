...
public class InventoryStep : UiTestStepBase
{
    ...
    private IEnumerator CheckMoveItem()
    {
        ...
        var startCellIsEmptyChecker2 = new CountIsEmptyChecker(Context, startItemLoc, 0);
        var cell_status2 = startCellIsEmptyChecker2.Check(); 

        var startCellIsEmptyChecker = new CountIsEmptyChecker(Context, startItemLoc, 9);
        var cell_status = startCellIsEmptyChecker.Check(); 
        Fail($"cell {startItemIndex} expected to be empty after moving, filled {cell_status}, {cell_status2}");
        // {"tag":"info","depth":0,"time":"2025-3-19 11:22:11","data":{"name":"Final Error","traceback":"cell 0 expected to be empty after moving, filled True, True"}}
        ...
        // var cell_status = startCellIsEmptyChecker.Check(); 
        // Fail($"cell {startItemIndex} expected to be empty after moving, filled {cell_status}");
